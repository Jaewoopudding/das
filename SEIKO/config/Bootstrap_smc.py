...
def aesthetic():
    ...
    config.sample.batch_size_per_gpu_available = 2

    config.reward_fn = 'aesthetic' # CLIP or imagenet or .... or .. 
    config.train.reward_lr = 1e-3
    ...
    config.debug = False
    config.sample_eta = 1.0
    config.train.kl_weight = 0.005
    
    config.train.num_outer_loop = 1 + 256
    config.good_prompt_prop = 0.25
    config.num_samples = [256] + [16] * 256  # 1k samples take 1h/gpu
...
def jpeg():
    ...
    config.train.kl_weight = 0.1
    config.train.reward_lr = 3e-4

    return config
    
def evaluate():
    config = aesthetic() # jpeg()

    config.prompt_fn = "eval_simple_animals"
    config.samping_prompt_fn = "eval_simple_animals"
    ...
    config.train.num_outer_loop = 1
    config.num_samples = [32]

    config.reward_model_path = "logs/test_2024.11.17_21.27.49 (jpeg Bootstrap smc)/rewards_models/MLP_outer_loop_63.pth"

    config.smc.num_particles = 16

    config.max_vis_images = 16

    config.good_prompt_prop = 0.0
